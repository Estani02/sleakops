...
import {Box, Button, Input, Select, Switch} from '@chakra-ui/react';
...
export default function PricingTableFilters({onFilterChange}: PricingTableFiltersProps) {
  ...
  const [showAllRequiredFields, setShowAllRequiredFields] = useState(false);

  const handleChange = (key: FilterKeys, value: string | number | boolean | undefined) => {
  ...
  const handleSwitchChange = () => {
    setShowAllRequiredFields((prev) => !prev);
    handleChange('allRequierdFields', !showAllRequiredFields);
  };

  const handleClearFilters = () => {
    setFilters({});
    onFilterChange({});
    setShowAllRequiredFields(false);
  };

  return (
    <Box className="mt-12 mb-4 flex flex-col justify-between gap-1.5 lg:gap-5">
      <Box className="flex items-center gap-2.5">
        <p className="text-sm">Complete fields</p>
        <Switch isChecked={showAllRequiredFields} size="md" onChange={handleSwitchChange} />
      </Box>
      <Box
        className="flex flex-col justify-between gap-1.5 lg:flex-row lg:items-center lg:gap-5"
        mb={4}
      >
        {/* <FilterIcon className="mt-2.5" size={36} /> */}
